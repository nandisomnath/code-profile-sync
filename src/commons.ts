...
export default class Commons {
  ...
  public async StartMigrationProcess(): Promise<boolean> {
    ...
    let customSettings: CustomConfig | null = null;
  ...
  public async SaveSettings(setting: ExtensionConfig): Promise<boolean> {
    ...
    keys.forEach(async keyName => {
      ...
      if (keyName.toLowerCase() !== "token") {
        if (config.get(keyName) !== setting[keyName as keyof ExtensionConfig]) {
          allKeysUpdated.push(config.update(keyName, setting[keyName as keyof ExtensionConfig], true));
        }
      }
    });
